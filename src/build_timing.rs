use std::{
     collections::HashMap, fs::File, mem, path::Path
};
...
pub struct BuildTiming {
    /// The file that `build-timing` writes build information to.
    ///
    ...
    /// Constants in the deny list, passed through [`BuildTimingBuilder::allow_const`].
    ///
    /// This set contains build constant identifiers that should be allow from the build process.
    pub allow_const: Vec<BuildTimingConst>,

    /// The output path where generated files will be placed.
    ///
    /// This specifies the directory where the build script will write its output. It's typically set using the `OUT_DIR` environment variable but can be customized using [`BuildTimingBuilder::out_path`].
    pub out_path: String,
...
impl BuildTiming {
    ...
    fn gen_header(&self) -> BtResult<()> {
        let desc = format!(
            r#"// Code automatically generated by `build-timing` (https://github.com/copie/build-timing), do not edit.
// Author: https://www.github.com/copie
// Generation time: {}
"#,
    ...
    fn gen_const(&mut self) -> BtResult<()> {
        ...
        self.build_pattern.rerun_if(self.allow_const.iter(), out_dir);
        let mut flag = HashMap::new();
        for const_name in self.hook_consts.iter() {
            if flag.contains_key(&const_name.to_string()) {
                continue;
            }
            flag.insert(const_name.to_string(), true);
            self.write_const(&const_name.to_string(), &const_name.build_val())?;
        }
        for const_name in self.allow_const.iter() {
            if flag.contains_key(&const_name.to_string()) {
                continue;
            }
            flag.insert(const_name.to_string(), true);
